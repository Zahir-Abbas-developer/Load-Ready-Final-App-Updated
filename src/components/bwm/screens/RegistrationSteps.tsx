...
import { X, Check, Upload, FileText, Hourglass, ChevronDown, CheckCircle2, ShieldCheck } from "lucide-react";

const US_STATES = [
  "Texas", "Oklahoma", "California", "Arizona", "New Mexico", "Louisiana",
  "Florida", "Georgia", "Nevada", "Colorado", "Kansas", "Missouri",
];

const EQUIPMENT = [
  "Reflective Cones", "Warning Flashers", "Height Pole", "Strobe Lights",
  "Two-way Radio", "First Aid Kit", "Fire Extinguisher",
];

function StepShell({
  step, total, title, children, onNext, nextDisabled = false, nextLabel = "Next",
}: {
  step: number; total: number; title: string; children: React.ReactNode;
  onNext: () => void; nextDisabled?: boolean; nextLabel?: string;
}) {
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <div className="px-6 pb-6 pt-2 border-t border-border bg-background">
        <PrimaryButton disabled={nextDisabled} onClick={onNext}>{nextLabel}</PrimaryButton>
      </div>
    </div>
  );
}
    ...
    <div>
      ...
      {selected.length > 0 && (
        <div className="flex flex-wrap gap-2 mt-3">
          {selected.map((s) => (
            <span key={s} className="inline-flex items-center gap-1 px-3 py-1.5 rounded-full bg-primary text-primary-foreground text-sm font-medium">
              {s}
...
function Field({ label, ...p }: { label: string } & React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <div className="mb-4">
      ...
      <input
        {...p}
        className="w-full h-13 px-4 py-3 rounded-xl bg-surface border border-border focus:outline-none focus:border-primary"
      />
    </div>
  );
}
...
export function PilotStep1() {
  ...
  return (
    <StepShell step={1} total={6} title="Area and states" onNext={() => go("pilot-step2")} nextDisabled={states.length === 0}>
      <p className="text-sm text-muted-foreground mb-4">choose the states/areas you can work on :</p>
      <MultiSelect label="Choose states/areas" options={US_STATES} selected={states} onChange={setStates} max={5} />
  ...
  return (
    <StepShell step={2} total={6} title="Personal information" onNext={() => go("pilot-step3")}>
      ...
      <ul className="mb-4 space-y-1">
        {rules.map((r) => (
          <li key={r.label} className={`flex items-center gap-2 text-xs ${r.ok ? "text-success" : "text-destructive"}`}>
      ...
      <div className="mb-4">
        <label className="block text-sm font-medium text-foreground mb-1.5">Government ID type</label>
        <select className="w-full h-13 px-4 rounded-xl bg-surface border border-border focus:outline-none focus:border-primary">
  ...
  return (
    <StepShell step={3} total={6} title="Vehicle information" onNext={() => go("pilot-step4")}>
      <div className="mb-4">
        ...
        <select className="w-full h-13 px-4 rounded-xl bg-surface border border-border focus:outline-none focus:border-primary">
          <option>Sedan</option><option>SUV</option><option>Pickup</option><option>Box truck</option>
        </select>
      </div>
...
export function PilotStep4() {
  ...
  return (
    <StepShell step={4} total={6} title="Certification and equipment" onNext={() => go("pilot-step5")}>
      <p className="text-sm text-muted-foreground mb-3">choose the equipment that included in your vehicle :</p>
      ...
      <div className="mb-4">
        <label className="block text-sm font-medium text-foreground mb-1.5">Certification level</label>
        <select className="w-full h-13 px-4 rounded-xl bg-surface border border-border">
          <option>Basic</option><option>Advanced</option><option>Premium</option><option>State-Certified</option>
        </select>
      </div>
...
export function PilotStep5() {
  ...
  return (
    <StepShell
      step={5} total={6} title="Payment information"
      nextLabel="Save and continue" nextDisabled={!agree}
      onNext={() => go("pilot-step6")}
    >
      <p className="text-sm text-muted-foreground mb-4">Add your bank details to receive your payouts securely.</p>
      ...
      <div className="flex gap-2 p-3 rounded-xl bg-success/10 border border-success/30">
        ...
        <p className="text-xs text-foreground">
          Bank information is encrypted and used only for secure payments through our payment processor.
        </p>
      </div>
    </StepShell>
  );
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <div className="px-6 pb-8 flex flex-col gap-2">
        <PrimaryButton onClick={() => go("pilot-approved")}>Get started (simulate approval)</PrimaryButton>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <div className="flex-1 px-6 overflow-y-auto">
        <div className="flex flex-col items-center text-center mb-6">
          ...
          <p className="text-muted-foreground text-sm">
            Your pilot car account is now active. You're now ready to accept offers and operate as a certified pilot car.
          </p>
        </div>
        <div className="space-y-3">
          {items.map((it, i) => (
            <label key={i} className="flex gap-3 p-3 rounded-xl bg-surface border border-border cursor-pointer">
              <input
                type="checkbox" checked={agreed[i]} onChange={() => toggle(i)}
      ...
      <div className="px-6 py-6 border-t border-border">
        <PrimaryButton disabled={!allOk} onClick={() => go("home")}>Get started</PrimaryButton>
      </div>
    </div>
  );
}
...
export function DispatcherStep1() {
  const { go, states, setStates } = useOnboarding();
  return (
    <StepShell step={1} total={4} title="Area and states" onNext={() => go("dispatcher-step2")} nextDisabled={states.length === 0}>
      <p className="text-sm text-muted-foreground mb-4">choose the states/areas you may work on :</p>
      <MultiSelect label="Choose states/areas" options={US_STATES} selected={states} onChange={setStates} max={5} />
    </StepShell>
  );
}
export function DispatcherStep2() {
  const { go } = useOnboarding();
  return (
    <StepShell step={2} total={4} title="Personal information" onNext={() => go("dispatcher-step3")}>
      <Field label="Full name" placeholder="Ex: John Carter" />
      <Field label="Phone number" placeholder="+1 ___ ___ ____" />
...
export function DispatcherStep3() {
  ...
  return (
    <StepShell
      step={3} total={4} title="Payment information"
      nextLabel="Save and continue" nextDisabled={!agree}
      onNext={() => go("dispatcher-step4")}
    >
      <p className="text-sm text-muted-foreground mb-4">Connect your bank account to pay pilot cars and receive invoices securely.</p>
      <Field label="Bank name" />
      <Field label="Account Holder Name" />
      <Field label="Account Number" inputMode="numeric" />
      <Field label="Routing Number" inputMode="numeric" />
      <label className="flex gap-2 items-start mt-2 mb-4 cursor-pointer">
        <input type="checkbox" checked={agree} onChange={(e) => setAgree(e.target.checked)} className="mt-1 h-4 w-4 accent-[var(--primary)]" />
        <span className="text-sm">I authorize ACH payments and charges according to the <span className="text-primary underline">terms</span>.</span>
      </label>
      <div className="flex gap-2 p-3 rounded-xl bg-success/10 border border-success/30">
        <ShieldCheck className="h-5 w-5 text-success shrink-0 mt-0.5" />
...
export function DispatcherStep4() {
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <div className="flex-1 px-6 overflow-y-auto">
        ...
        <div className="space-y-3">
          {items.map((it, i) => (
            <label key={i} className="flex gap-3 p-3 rounded-xl bg-surface border border-border cursor-pointer">
              <input
                type="checkbox" checked={agreed[i]}
      ...
      <div className="px-6 py-6 border-t border-border">
        <PrimaryButton disabled={!agreed.every(Boolean)} onClick={() => go("home")}>Get started</PrimaryButton>
      </div>
    </div>
  );
}
...
// Dispatch helper
export const STEP_MAP: Record<OnboardingStep, () => React.ReactElement | null> = {
  splash: () => null, slide1: () => null, slide2: () => null, slide3: () => null,
  role: () => null, signup: () => null, otp: () => null,
  "pilot-step1": PilotStep1, "pilot-step2": PilotStep2, "pilot-step3": PilotStep3,
  "pilot-step4": PilotStep4, "pilot-step5": PilotStep5, "pilot-step6": PilotStep6,
  "pilot-approved": PilotApproved,
  "dispatcher-step1": DispatcherStep1, "dispatcher-step2": DispatcherStep2,
  "dispatcher-step3": DispatcherStep3, "dispatcher-step4": DispatcherStep4,
  home: () => null,
  admin: () => null,
};
